 ...
 */
 class Subteraskampus extends CI_Controller{
    ...
    public function tambah()
    {
        ...
        $html['content']  = $this->load->view('admin/tambah-teraskampus',$eusi, true);
    ...
    { 
        ...
        $config['upload_path'] = './pict/teraskampus/';
        ...
        if($_FILES['userfile']['name'] == "") {                                 
            $datanya = array( 
                            'id_teraskampus'          => $this->input->post('id_teraskampus'),
                            'id'                      => $this->input->post('id'),
                            'waktu_post'              => $this->input->post('waktu_post'),
                            'nama_sub_teraskampus'    => $this->input->post('nama_sub_teraskampus'),
                            'deskripsi'               => $this->input->post('deskripsi')
            );
            $this->msubteraskampus->tambah($datanya);
...
//              $this->_create_thumbnail($data["file_name"]); 
    ...
    public function detail($id=null)
    {
        ...
        $menu['tampil']   = $this->mkomunitas->tampil();
        $menu['tompel']   = $this->mteraskampus->tampil();
        $data['detail']   = $this->msubteraskampus->get_id($id);
        $html             = array();
        $html['header']   = $this->load->view('admin/template/header',null, true);
        $html['topbar']   = $this->load->view('admin/template/topbar',null, true);
        $html['menu']     = $this->load->view('admin/template/menu',$menu, true);
        $html['content']  = $this->load->view('admin/detail-subteraskampus',$data, true);
        $html['js']       = $this->load->view('admin/template/js',null, true);
        $this->load->view('admin/template/template',$html);
    }
    ...
    public function ubah($id){
        ...
        $menu['tampil']   = $this->mkomunitas->tampil();
        $menu['tompel']   = $this->mteraskampus->tampil();
        $data['tampil']   = $this->mteraskampus->tampil();
        $data['detail']   = $this->msubteraskampus->get_id($id);
        $html             = array();
        $html['header']   = $this->load->view('admin/template/header',null, true);
        $html['topbar']   = $this->load->view('admin/template/topbar',null, true);
        $html['menu']     = $this->load->view('admin/template/menu',$menu, true);
        $html['content']  = $this->load->view('admin/ubah-teraskampus',$data, true);
        $html['js']       = $this->load->view('admin/template/js',null, true);
        $this->load->view('admin/template/template',$html);   
    }

    public function ubah_teraskampus(){
        $config['file_name'] = 'pict_'.date('Y_m_d_H_i_s');
        $config['upload_path'] = './pict/teraskampus/';
        $config['allowed_types'] = 'gif|GIF|jpg|JPG|jpeg|JPEG|png|PNG';
        $config['max_size'] = '8000';
        ...
        if($_FILES['userfile']['name'] == "") {                                 
            ...
            $condition['id_sub_teraskampus'] = $this->input->post('id_sub_teraskampus'); 
            $this->msubteraskampus->ubah_data($datanya, $condition);
            redirect('secuser/home');
        }else{
            if (!$this->upload->do_upload()){
                echo 'Error Bos';
    ...
    public function hapus()
    {
        ...
        $h=$this->msubteraskampus->hapus($id); 
        if($h==1451)
